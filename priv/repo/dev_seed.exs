...
defmodule Inserter do
  ...
  def insert_messages(previous) do
    current = add_minutes(previous, -1 *  @minutes_between_messages)
...
# # Insert these messages with random usernames.
# messages
# |> Enum.map(fn message ->
#   random_nick = Enum.random(nicknames)
...
#   random_timestamp =
#     {:ok, _} =
#     Ircois.Data.store_message(%{:from => random_nick, :content => message, :channel => channel})
# end)
